...
class GuidelineViewSetTestCase(TestCase):
    def setUp(self):
        ...
        self.guideline1 = Guideline.objects.create(title='Guideline 1', description='Description 1')
        self.guideline2 = Guideline.objects.create(title='Guideline 2', description='Description 2')
    ...
    def test_retrieve_guideline(self):
        response = self.client.get(reverse('guideline-detail', args=[self.guideline1.id]))
    ...
    def test_update_guideline(self):
        ...
        response = self.client.put(reverse('guideline-detail', args=[self.guideline1.id]), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_partial_update_guideline(self):
        ...
        response = self.client.patch(reverse('guideline-detail', args=[self.guideline1.id]), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.guideline1.refresh_from_db()
        self.assertEqual(self.guideline1.description, 'Updated Description')

    def test_delete_guideline_not_allowed(self):
        response = self.client.delete(reverse('guideline-detail', args=[self.guideline1.id]))
...
class ContentListViewTestCase(TestCase):
    def setUp(self):
        ...
        self.content1 = Content.objects.create(title='Content 1', file='content1.txt', author=self.user)
        self.content2 = Content.objects.create(title='Content 2', file='content2.txt', author=self.user)
...
class ContentUploadViewTestCase(TestCase):
    ...
    def test_upload_content_success(self):
        ...
        self.assertEqual(ReviewItem.objects.filter(content=content).count(), Guideline.objects.count())

    def test_upload_content_with_same_title(self):
        Content.objects.create(title='Test Content', file='testfile.txt', author=self.user)
        ...
        self.assertEqual(response.data['error'], 'Content with the same title already exists. Please choose a unique title or update existing content.')
    ...
    def test_upload_content_with_non_allowed_extension(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['file'][0], 'Invalid file type. Only images, Word documents, TXT, and PDF files are allowed.')
...
class ContentReviewStatusViewTestCase(TestCase):
    def setUp(self):
        ...
        self.content = Content.objects.create(title='Test Content', file='testfile.txt', author=self.user)
        self.guideline1 = Guideline.objects.create(title='Guideline 1', description='Description 1')
        self.guideline2 = Guideline.objects.create(title='Guideline 2', description='Description 2')
        self.review_item1 = ReviewItem.objects.create(content=self.content, guideline=self.guideline1, status=ReviewItem.StatusChoices.PASSED)
        self.review_item2 = ReviewItem.objects.create(content=self.content, guideline=self.guideline2, status=ReviewItem.StatusChoices.PENDING)
    ...
    def test_get_review_status(self):
        ...
        expected_data = ReviewItemSerializer([self.review_item1, self.review_item2], many=True).data
...
class ContentReviewUpdateAPIViewTestCase(TestCase):
    def setUp(self):
        ...
        self.guideline1 = Guideline.objects.create(title='Guideline 1', description='Description 1')
        self.guideline2 = Guideline.objects.create(title='Guideline 2', description='Description 2')
        self.review_item1 = ReviewItem.objects.create(content=self.content, guideline=self.guideline1, status=ReviewItem.StatusChoices.PENDING)
        self.review_item2 = ReviewItem.objects.create(content=self.content, guideline=self.guideline2, status=ReviewItem.StatusChoices.PENDING)

    def test_update_review_item_success(self):
        url = reverse('content-review-update', kwargs={'content_id': self.content.pk, 'review_item_id': self.review_item1.pk})
        data = {'status': 'PASS'}
    ...
    def test_update_review_item_nonexistent_content(self):
        url = reverse('content-review-update', kwargs={'content_id': 999, 'review_item_id': self.review_item1.pk})
        data = {'status': 'PASS'}
    ...
    def test_update_review_item_nonexistent_review_item(self):
        url = reverse('content-review-update', kwargs={'content_id': self.content.pk, 'review_item_id': 999})
        data = {'status': 'PASS'}

        response = self.client.put(url, data, format='json')

        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data['error'], 'No Review item matches the given query.')
